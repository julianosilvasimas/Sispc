...
export class PrintdeliberacaoComponent implements OnInit {
  ...
  inserir(ask, bid, epoch, id, quote, symbol){

    try{
      console.log(ask,' - ',bid,' - ',epoch,' - ',id,' - ',quote,' - ',symbol)
        this.service.InputTeste(ask, bid, epoch, id, quote, symbol)
        .subscribe(
          response => {
  ...
  connBinary(){
    ...
    setInterval( function() {
      ...
      if (tique[0].quote != this.tiqueCheck){
        console.log(tique[0].quote);
        ask = tique[0].ask;
        bid = tique[0].bid;
        epoch = tique[0].epoch;
        id = tique[0].id;
        quote = tique[0].quote;
        symbol = tique[0].symbol;

        //this.inserir(ask, bid, epoch, id, quote, symbol)
      
      //console.log(tiqueCheck)
      this.tiqueCheck = tique[0].quote;
  ...
  criaPDF(modelo: string, delib: string, rua :string, bairro: string, cidade: string, 
    ...
    if(modelo==='DF' || modelo==='DI'){
      win.document.write('<div class="marcapa">'+
        ...
        '<spam class="mesano">'+("00" + datJulgado.getMonth()).slice(-2)+'</spam>'+
