    ...
    getDataRecordsArrayFromCSVFile(csvRecordsArray, headerLength, 
        ...
        for (let i = 0; i < csvRecordsArray.length; i++) {
            ...
            if(validateHeaderAndRecordLengthFlag && data.length != headerLength){
                if(data==""){
                    alert("Linha extra em branco está presente na linha de número "+i+", Remova por favor.");
                    return null;
                }else{
                    alert("Registro da linha de número  "+i+"contém "+data.length+" colunas, e não está combinando com o comprimento do cabeçalho de :"+headerLength);
                    return null;
                }
            }
